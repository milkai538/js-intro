...
Imagine you are sorting a child's toys into buckets. The primary rule of sorting, is simply putting everything where it belongs. You don't want the squares in the ball bucket, so you label which item goes into which bucket. Ex: Balls go into the biggest bucket.
...
```javascript
// Project development led by Aidan "Ferris" Milk.
